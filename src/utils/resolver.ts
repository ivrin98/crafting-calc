...
export type ItemNeed = {
  ...
  outputqty?: number;
  crafts?: number;
  craftableFromOwnedRaw?: number;
};

export type ResolveResult = {
  raw: Record<string, ItemNeed>;
  craftable: Record<string, ItemNeed>;
};

function getOwned(name: string, inventory: Record<string, number>): number {
...
function addRow(target: Record<string, ItemNeed>, row: ItemNeed) {
...
function calculateCraftableFromOwnedRaw(
  itemName: string,
  recipes: RecipeMap,
  inventory: Record<string, number>
): number {
  const recipeKey = findRecipeKey(itemName, recipes);
  if (!recipeKey) return 0;
  ...
  if (!hasRawMaterial || maxCrafts === Infinity) return 0;
...
export function finalizeResolvedResult(
  result: ResolveResult,
  recipes: RecipeMap,
  inventory: Record<string, number>
): ResolveResult {
  ...
  for (const row of Object.values(result.craftable)) {
    row.owned = getOwned(row.name, inventory);
    row.missing = Math.max(0, row.needed - row.owned);
    row.craftableFromOwnedRaw = calculateCraftableFromOwnedRaw(
      row.name,
      recipes,
      inventory
    );
